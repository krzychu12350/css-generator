<template>
  <section class="py-14">
    <div class="max-w-screen-xl mx-auto px-4 text-gray-600 md:px-8">
      ...
      <div class="mt-12">
        <ul
          class="grid gap-x-12 divide-y [&>.feature-1]:pl-0 sm:grid-cols-2 sm:gap-y-8 sm:divide-y-0 lg:divide-x lg:grid-cols-3 lg:gap-x-0"
        >
          <li
            v-for="(item, idx) in features"
            :key="idx"
            :class="`feature-${idx + 1} space-y-3 py-8 lg:px-12 sm:py-0`"
          >
            <div
              class="w-12 h-12 border text-indigo-600 rounded-full flex items-center justify-center"
              v-html="item.icon"
            ></div>
